...
window.addEventListener("load", function() {
  (function($) {
    ...
    ShoppingCart.prototype.bindClickAddToCart = function() {
      ...
      this.$element.find('.js-add-to-cart').each(function() {
        $(this).click(function() {
          ...
          self.itemVariantId = $(this).data('variant-id');
          ...
          if(self.isValidCart() && self.isPositiveNumber(self.item_qty)) {
            self.addToCart(this);
          }
        });
      });
    ...
    ShoppingCart.prototype.isValidCart = function () {
      var isValid = true;
      if ($('#product-variants').length > 0) {
        $('.product-variant-items').each( function (){
